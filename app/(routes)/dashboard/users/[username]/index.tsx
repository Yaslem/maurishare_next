...
interface EditProfileFormProps {
  initialData: userResponse & { can_create_post: boolean, can_update_post: boolean, can_delete_post: boolean, can_create_comment: boolean, totalPosts: number, totalReads: number } | null,
...
export default function EditProfileForm({ initialData, onUpdateProfile, onUploadUserPhoto }: EditProfileFormProps) {
  const [permissions, setPermissions] = useState({
    can_create_post: initialData?.can_create_post || false,
    can_update_post: initialData?.can_update_post || false,
    can_delete_post: initialData?.can_delete_post || false,
    can_create_comment: initialData?.can_create_comment || false
  })
                ...
                <div className={"flex flex-col w-full gap-4"}>
                    <div className={"flex items-center gap-4 flex-wrap"}>
                        <div className={"flex flex-col gap-3"}>
                            ...
                            <p className={"font-medium"}>{initialData?.totalPosts || 0}</p>
                        </div>
                        <div className={"flex flex-col gap-3"}>
                            <div className={"flex gap-3 items-center"}>
                            ...
                            <p className={"font-medium"}>{initialData?.totalReads || 0}</p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
                        ...
                        <div className={"flex flex-col gap-3"}>
                            ...
                            <div className={"flex flex-wrap gap-3"}>
                                <div className={"flex items-center gap-2"}>
                                    ...
                                    <input defaultChecked={permissions.can_create_post} onChange={e => setPermissions({...permissions, can_create_post: e.target.checked})} name={"can_create_post"} id={"can_create_post"} type={"checkbox"}/>
                                </div>
                                <div className={"flex items-center gap-2"}>
                                    <label htmlFor={"can_update_post"}>تحديث مقالة</label>
                                    <input defaultChecked={permissions.can_update_post} onChange={e => setPermissions({...permissions, can_update_post: e.target.checked})} name={"can_update_post"} id={"can_update_post"} type={"checkbox"}/>
                                </div>
                                <div className={"flex items-center gap-2"}>
                                    <label htmlFor={"can_delete_post"}>حذف مقالة</label>
                                    <input defaultChecked={permissions.can_delete_post} onChange={e => setPermissions({...permissions, can_delete_post: e.target.checked})} name={"can_delete_post"} id={"can_delete_post"} type={"checkbox"}/>
                                </div>
                                <div className={"flex items-center gap-2"}>
                                    <label htmlFor={"can_create_comment"}>نشر تعليق</label>
                                    <input defaultChecked={permissions.can_create_comment} onChange={e => setPermissions({...permissions, can_create_comment: e.target.checked})} name={"can_create_comment"} id={"can_create_comment"} type={"checkbox"}/>
                                </div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                name="bio"
                                maxLength={150}
                                className="input-box h-64 lg:h-40 resize-none leading-7 mt-5 pr-5"
                            ></textarea>
                            ...
                            <div className="md:grid md:grid-cols-2 gap-x-6">
                                {
                                    Object.keys(initialData?.socialLinks || {}).map((key: string, i: number) => {
